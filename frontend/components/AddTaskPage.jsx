...
const AddTaskPage = () => {
    ...
    // Get selected case from dashboard filter
    ...
    useEffect(() => {
        if (user && !user.Projects && !hasFullUserData) {
            ...
            loadUserWithProjects().then(fullUser => {
                ...
                console.error('AddTaskPage - Error loading user with projects:', error);
                setProjectsLoading(false);
    ...
    // Auto-select case/project when selectedCase is available from dashboard filter
    useEffect(() => {
        if (selectedCase) {
            const projectIdString = String(selectedCase.project_id);
            // Only update if different to prevent loops
            if (selectedProject !== projectIdString) {
                setSelectedProject(projectIdString);
            }
        }
    }, [selectedCase, selectedProject]);
                                ...
                                <div className="grid grid-cols-[auto,1fr] gap-6 items-center">
                                    ...
                                    <Select onValueChange={(value) => {
                                        if (value !== "no-projects") {
                                            ...
                                            const project = userWithProjects?.Projects?.find(p => p.project_id == value);
                                            if (project) {
