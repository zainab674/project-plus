...
export default function TabBar() {
  ...
  return (
    <div className="bg-gray-300 sticky top-16 z-30">
      <div
        ref={tabBarRef}
        className="flex-1 overflow-x-auto overflow-y-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
      >
        <div className="flex items-end h-9 bg-gray-300 px-2">
          {tabs.map((tab, index) => {
            const isActive = tab.id === activeTabId;
            const isFirst = index === 0;
            const isLast = index === tabs.length - 1;
            ...
            return (
              <div
                ...
                onClick={() => handleTabClick(tab.id)}
                className={`
                  group relative flex items-center h-7
                  cursor-pointer
                  transition-all duration-200
                  min-w-[100px] max-w-[200px]
                  ${tabColor}
                  ${isFirst ? '' : '-ml-3'}
                  ${isActive ? 'border-2 border-white shadow-md' : ''}
                `}
                style={{
                  clipPath: isFirst && isLast
                    ? 'polygon(8px 0, calc(100% - 8px) 0, 100% 100%, 0 100%)'
                    : isFirst
                      ? 'polygon(8px 0, calc(100% - 16px) 0, 100% 100%, 0 100%)'
                      : isLast
                        ? 'polygon(16px 0, calc(100% - 8px) 0, 100% 100%, 0 100%)'
                        : 'polygon(16px 0, calc(100% - 16px) 0, 100% 100%, 0 100%)',
                  paddingLeft: isFirst ? '18px' : '24px',
                  paddingRight: isLast ? '18px' : '24px',
              ...
              >
                ...
                <span
                  className={`
                    text-xs truncate flex-1 text-center font-medium
                    text-white
                ...
                <button
                  onClick={(e) => handleCloseTab(e, tab.id)}
                  className={`
                    flex-shrink-0 w-3.5 h-3.5 ml-2
                    flex items-center justify-center
                    rounded-sm
                    transition-opacity duration-200
                    ${isActive ? 'opacity-70 hover:opacity-100 hover:bg-black/20' : 'opacity-0 group-hover:opacity-60 hover:!opacity-100'}
                    ${tabs.length === 1 ? '!opacity-0 cursor-default' : ''}
                  `}
                  disabled={tabs.length === 1}
                  title={tabs.length === 1 ? 'Cannot close the last tab' : 'Close tab'}
                >
