...
import { Search, GitBranch, Eye, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useUser } from '@/providers/UserProvider';
import { useProjectState } from '@/hooks/useProjectState';
import { useTimelineState } from '@/hooks/useTimelineState';
import { useDashboardFilter } from '@/providers/DashboardFilterProvider';
import CaseComprehensiveView from '@/components/case/CaseComprehensiveView';
...
export default function FlowchartPage() {
  ...
  useEffect(() => {
    if (selectedCase && projectState.projects) {
      ...
      // If project found in list but doesn't have full data, fetch it
      if (project) {
        // Check if we need more detailed data (Tasks, Members, Clients, etc.)
        const hasFullData = project.Tasks && project.Members && project.Clients;
        
        if (!hasFullData) {
          setIsLoadingProject(true);
          getProjectRequest(project.project_id)
            .then(res => {
              if (res?.data?.project) {
                timelineState.setSelectedProjectForTimeline(res.data.project);
                setSelectedCase(res.data.project);
              }
            })
            .catch(err => {
              console.error('Error loading project details:', err);
              // Fall back to basic project data
              timelineState.setSelectedProjectForTimeline(project);
            })
            .finally(() => {
              setIsLoadingProject(false);
            });
        } else {
          // Already has full data, just set it
          if (!timelineState.selectedProjectForTimeline || 
              timelineState.selectedProjectForTimeline.project_id !== project.project_id) {
            timelineState.setSelectedProjectForTimeline(project);
          }
        }
      } else if (selectedCase.project_id) {
        // Project not in list, fetch it directly
        setIsLoadingProject(true);
        getProjectRequest(selectedCase.project_id)
          .then(res => {
            if (res?.data?.project) {
              timelineState.setSelectedProjectForTimeline(res.data.project);
              setSelectedCase(res.data.project);
    ...
    } else if (!selectedCase && timelineState.selectedProjectForTimeline) {
      // If no case is selected in top nav but we have one selected locally, clear it
      timelineState.setSelectedProjectForTimeline(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedCase, projectState.projects]);
  ...
  const selectedProjectForTimeline = timelineState.selectedProjectForTimeline;
      ...
      <div className="max-w-7xl mx-auto p-6">
        {!selectedProjectForTimeline ? (
          ...
          <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredProjects?.map((project, index) => (
                  <button
                    ...
                    className="text-left p-4 border border-gray-200 rounded-lg hover:border-blue-300 hover:shadow-md transition-all duration-200 group bg-white"
                  >
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-semibold text-gray-800 group-hover:text-blue-600 transition-colors truncate">
                        {project.name}
                      </h3>
                      <span className={`ml-2 text-xs px-2 py-1 rounded-full flex-shrink-0 ${project.status === 'Active'
            ...
            {!projectState.projectsLoading && filteredProjects?.length === 0 && (
